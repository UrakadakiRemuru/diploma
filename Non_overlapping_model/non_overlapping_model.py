...
def find_params_for_transformed_ellipse(S_ij: np.array, c: tuple) -> ellipse:
    """Реализует приведение уравнения 2-ого порядка для эллипса к каноническому виду. Возвращает объект класса эллипса.
...
def not_overlapping_border(elp: ellipse, length: float, width: float):
    """Условие нахождения внутри заданного прямоугольника, исключающее пересечение с границей прямоугольника.
    Центр прямоугольника должен совпадать с началом координат. \n
    Функция возвращает логическую переменную: в случае
    пересечения с границей прямоугольника или выхода за пределы внутренности прямоугольника возвращается False, иначе -
    True.
    :param elp: Эллипс.
    ...
    s_list = [length / 2 if i < 2 else width / 2 for i in range(4)]
    marker = False
