...
def transform(elp: ellipse, c: Optional[Tuple] = None) -> tuple:
    ...
    if c is None:
        return tuple(
            np.linalg.multi_dot([inverse(sqrt_diagonal(P(elp.a, elp.b))), transpose(Q(elp.angle)), transpose(elp.coord_to_array())]))
    else:
        return tuple(
            np.linalg.multi_dot([inverse(sqrt_diagonal(P(elp.a, elp.b))), transpose(Q(elp.angle)), transpose(np.array(list(c)))]))
